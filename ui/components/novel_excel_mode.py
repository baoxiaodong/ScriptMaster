...
def render_novel_excel_mode(llm_service, file_handler: FileHandler):
    ...
    try:
        ...
        target_df = st.session_state.get('selected_df', df)
        total_episodes = st.session_state.get('total_episodes', 20)

        # ================= 🕳️ 魔法黑洞容器 =================
        # 我们用一个 empty 占位符把下方所有交互区包裹起来
        main_ui = st.empty()

        # ================= 🚀 大模型生成执行区 =================
        if is_gen:
            # 💥 核心大招：一旦进入生成状态，瞬间炸毁黑洞容器！
            # 浏览器会立刻（在0.01秒内）抹除掉之前的配置区和按钮，绝对不留残影！
            main_ui.empty()
            ...
            if not has_outline:
                ...
                # 阶段 B：从大纲生成分镜（大纲只读组件会渲染在这里，进度条上方）
                ...
                # ================= 🎨 交互界面渲染区 =================
        # 只有非生成状态，才会在容器内渲染这些组件
        with main_ui.container():

            # 【状态 0：初始页面】 -> 显示选择器和生成配置按钮
                    ...
                    st.markdown(
                        f"""<div style="background-color: #FFF9F0; border: 1px solid #F0D9A0; padding: 10px; border-radius: 8px; text-align: center;">
                            <p style="margin: 0; font-size: 14px; color: #E3700D; font-weight: bold;">
                                分 {batch_count} 次生成<br>
                                <span style="color: #C68A1E; font-size: 11px; font-weight: normal;">多线程并行加速</span>
                            </p>
            ...
            # 【状态 1：大纲完成】 -> 显示大纲编辑器
                ...
                if st.button("🗑️ 返回初始配置 (清空大纲)", use_container_width=True):
                    st.session_state.novel_outline = None
                    st.session_state.novel_results = {}
                    if 'selected_df' in st.session_state: del st.session_state['selected_df']
                    st.rerun()

                render_novel_outline_section(llm_service, target_df, total_episodes)

            # 【状态 2：分镜完成】 -> 显示大纲（只读）+ 分镜结果
            elif has_results:
...
def _execute_generation_flow(llm_service, df, total_episodes: int = 20):
    ...
    progress_bar = st.progress(0, text=f"🚀 正在分析原著，构思 {total_episodes} 集骨架...")
    try:
        with st.spinner():
...
def _execute_scripts_generation(llm_service, df, total_episodes: int = 20):
    ...
    progress_bar = st.progress(0, text=f"🚀 准备生成 {total_episodes} 集分镜（预计{estimated_minutes}分钟）...")
    try:
